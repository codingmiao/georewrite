 ...
 */
public abstract class PrtreeIndex<T> {
	 ...
	 *            需要索引的对象Collection
	 */
	public PrtreeIndex(Collection<T> features) {
		ArrayList<PrtreeLeafNode> leafNodes = new ArrayList<>(features.size());
		for (T node : features) {
			PrtreeLeafNode leafNode = feature2Node(node);
			if (null != leafNode) {
	...
	private PrtreeLeafNode feature2Node(T feature) {
		Geometry geo = feature2Geometry(feature);
		...
		PrtreeLeafNode node = new PrtreeLeafNode(feature, xmin, ymin, w, h);
		return node;
	}
	...
	/**
	 * 通过feature获取其相应的geometry
	 * 
	 * @param feature
	 *            feature
	 * @return
	 */
	protected abstract Geometry feature2Geometry(T feature);
}
